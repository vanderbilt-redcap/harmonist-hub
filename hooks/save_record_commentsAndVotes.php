        ...
        if($request['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                if(!empty($comment['extra_revfile1'])){
                    $gd_files .= "<li>".\Vanderbilt\HarmonistHubExternalModule\getFileLink($module, $comment['extra_revfile1'],'',0,$secret_key,$secret_iv,$people['record_id'],"")."</li>";
                }
                if(!empty($comment['extra_revfile2'])){
                    $gd_files .= "<li>".\Vanderbilt\HarmonistHubExternalModule\getFileLink($module, $comment['extra_revfile2'],'',0,$secret_key,$secret_iv,$people['record_id'],"")."</li>";
                }
                $gd_files .= "</ol>";
                ...
                $url = $module->getUrl("index.php?token=".$people['access_token']."&option=hub&record=".$comment['sop_id']."&pid=".IEDEA_PROJECTS);
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$request['request_type']].' Request  #'.$request['request_id'].'</h2>
                            ...
                            <p><span style="color: #999999; font-size: 11px;">Want to stop following this Request? Here\'s a <a href="'.$module->getUrl('index.php?pid='.IEDEA_PROJECTS.'&token='.$people['access_token'].'&option=unf&record='.$request['request_id']).'">quick link to visit the Hub</a>.</span></p>
                            ';
