...
if(($comment[$instrument.'_complete'] == '2' || $vanderbilt_emailTrigger->getEmailTriggerRequested()) && $instrument == 'comments_and_votes'){
    ...
    if(!empty($request)){
        ...
        foreach ($request['responding_region'] as $instanceId => $resp_region){
            ...
            if(array_key_exists('voteregion_y',$voting_region) && $voting_region['voteregion_y'] == "1" && array_key_exists('region_vote_status',$voting_region) && (!array_key_exists($instanceId,$voting_region['region_vote_status']) ||  $request['region_vote_status'][$instanceId] == "")){
        ...
        if($request['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$request['request_type']].' Request  #'.$request['request_id'].'</h2>
                            ...
                            <p><span style="color: #999999; font-size: 11px;">Want to stop following this Request? Here\'s a <a href="'.$this->getUrl('index.php').'&NOAUTH&pid='.$pidsArray['PROJECTS'].'&option=unf&record='.$request['request_id'].'">quick link to visit the Hub and unfollow this request</a>.</span></p>
                            ';
