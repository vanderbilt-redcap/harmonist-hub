        ...
        if($request['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$request['request_type']].' Request  #'.$request['request_id'].'</h2>
                            ...
                            '.$gd_files.'
                            <p>&nbsp;</p>
                            <p><span style="color: #999999; font-size: 11px;">This email has been automatically generated by the '.$settings["hub_name"].' Hub system (<a href="'.$settings['hub_organization'].'">'.$settings['hub_organization'].'</a>). You are receiving this email because you signed up to follow this Request. If someone incorrectly submitted this request on your behalf or if you believe you received this email in error, please contact <a href="mailto:'.$settings["hub_contact_email"].'">'.$settings["hub_contact_email"].'</a>.</span></p>
                            <p><span style="color: #999999; font-size: 11px;">Want to stop following this Request? Here\'s a <a href="'.$this->getUrl('index.php').'&NOAUTH&pid='.$pidsArray['PROJECTS'].'&token='.$people['access_token'].'&option=unf&record='.$request['request_id'].'">quick link to visit the Hub</a>.</span></p>
                            ';
                \Vanderbilt\HarmonistHubExternalModule\sendEmail(strtolower($people['email']), $settings['accesslink_sender_email'], $settings['accesslink_sender_name'], $subject, $message,$people['record_id'],"New request feedback posted", $pidsArray['RMANAGER']);
            }
        }
    }
}
