        ...
        if($request['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$request['request_type']].' Request  #'.$request['request_id'].'</h2>
                            ...
                            <p><span style="color: #999999; font-size: 11px;">Want to stop following this Request? Here\'s a <a href="'.$this->getUrl('index.php').'&NOAUTH&pid='.$pidsArray['PROJECTS'].'&option=unf&record='.$request['request_id'].'">quick link to visit the Hub</a>.</span></p>
                            ';
