        ...
        if($sop['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$sop['request_type']].' Data Request  #'.$sop['record_id'].'</h2>
                                ...
                                <p><span style="color: #999999; font-size: 11px;">Want to stop following this Data Request? Here\'s a <a href="'.$module->getUrl('index.php?pid='.IEDEA_PROJECTS.'&token='.$people['access_token'].'&option=und&record='.$sop['record_id']).'">quick link to visit the Hub</a>.</span></p>
                                ';

                sendEmail(strtolower($people['email']), $settings['accesslink_sender_email'], $settings['accesslink_sender_name'], $subject, $message,$people['record_id'],"New data request feedback posted",IEDEA_SOP);
            }
        }
    }
}
