    ...
    if(!empty($sop)){
        if($sop['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $url = $this->getUrl("index.php?token=".$people['access_token']."&option=sop&record=".$comment['sop_id']."&pid=".IEDEA_PROJECTS);
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$sop['request_type']].' Data Request  #'.$sop['record_id'].'</h2>
                                ...
                                <p><span style="color: #999999; font-size: 11px;">Want to stop following this Data Request? Here\'s a <a href="'.$this->getUrl('index.php?pid='.IEDEA_PROJECTS.'&token='.$people['access_token'].'&option=und&record='.$sop['record_id']).'">quick link to visit the Hub</a>.</span></p>
                                ';
