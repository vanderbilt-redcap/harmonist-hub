    ...
    if(!empty($sop)){
        if($sop['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $url = $this->getUrl("index.php")."&NOAUTH&option=sop&record=".$comment['sop_id']."&pid=".$pidsArray['PROJECTS'];
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$sop['request_type']].' Data Request  #'.$sop['record_id'].'</h2>
                                ...
                                '.$gd_files.'
                                <p>&nbsp;</p>
                                <p><span style="color: #999999; font-size: 11px;">This email has been automatically generated by the '.$settings["hub_name"].' Hub system'.$hub_organization.'. You are receiving this email because you signed up to follow this Data Request. If someone incorrectly submitted this request on your behalf or if you believe you received this email in error, please contact <a href="mailto:'.$settings["hub_contact_email"].'">'.$settings["hub_contact_email"].'</a>.</span></p>
                                <p><span style="color: #999999; font-size: 11px;">Want to stop following this Data Request? Here\'s a <a href="'.$this->getUrl('index.php').'&NOAUTH&pid='.$pidsArray['PROJECTS'].'&option=und&record='.$sop['record_id'].'">quick link to visit the Hub</a>.</span></p>
                                ';
