    ...
    <div>
        <table class="table table_requests sortable-theme-bootstrap" data-sortable id="table_archive">
            ...
            if(!empty($request_reject)) {
                ...
                $user_req_header = \Functions\getRequestHeader($regions, $current_user['person_region'], $settings['vote_grid'], '2','archive');

                $requests_counter = 0;
                foreach ($request_reject as $req) {
                    $user_req_body .= \Functions\getHomeRequestHTML($module, $req, $regions, $request_type_label, $current_user, 2, $settings['vote_visibility'], $settings['vote_grid'],'none','archive');
                    if($user_req_body != ""){
                        $requests_counter++;
                    }
                }
