    ...
    if (($_REQUEST['type'] != "" && $req['request_type'] == $_REQUEST['type']) || $_REQUEST['type'] == "") {
        if (!hideRequestForNonVoters($settings, $req, $person_region)) {
            if (showClosedRequest($settings, $req, $current_user['person_region'])) {
                //COMPLETED REQUESTS
                $completed_req .= getRequestHTML($module, $req, $regions, $request_type_label, $current_user, 1, $settings['vote_visibility'], $settings['vote_grid'], '');
            } else if (showPendingRequest($req['request_id'], $req, $current_user['person_region']) && $current_user['pendingpanel_y'][0] == '1' && $req['region_response_status'][$current_user['person_region']] != '2') {
                //PENDING REQUESTS
                $pending_req .= getRequestHTML($module, $req, $regions, $request_type_label, $current_user, 0, $settings['vote_visibility'], $settings['vote_grid'], '');
            } else if (showOpenRequest($req, $current_user['person_region']) && $req['region_response_status'][$current_user['person_region']] != '2') {
                //OPEN REQUESTS
                $current_req .= getRequestHTML($module, $req, $regions, $request_type_label, $current_user, 0, $settings['vote_visibility'], $settings['vote_grid'], '');
            }
        }
    }
}
