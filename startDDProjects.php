...
$projects_array_sql = array(
    ...
    $pidsArray['SOP']=>array(
        ...
        'sop_finalize_person' => array (
            'query' => "SELECT DISTINCT a.record, CONCAT(a.value, ' ', b.value) AS VALUE FROM [data-table:".$pidsArray['PEOPLE']."] a LEFT JOIN redcap_data b on b.project_id = ".$pidsArray['PEOPLE']." and b.record = a.record and b.field_name = 'lastname' LEFT JOIN [data-table:".$pidsArray['PEOPLE']."] c on c.project_id = ".$pidsArray['PEOPLE']." and c.record = a.record WHERE a.field_name = 'firstname' and a.project_id = ".$pidsArray['PEOPLE']." and ((c.field_name = 'harmonist_perms' AND c.value = '1') OR (c.field_name = 'harmonistadmin_y' AND c.value = '1')) ORDER BY a.value, b.value",
            'autocomplete' => '0',
            'label' => ""
        ),
        'data_region' => array (
