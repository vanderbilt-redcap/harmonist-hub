...
if(strtotime($settings['publications_lastupdate']) < $today || $settings['publications_lastupdate'] == "" || $isAdmin) {
    ...
    if (!empty($concepts)) {
        ...
        foreach ($extra_outputs as $output) {
            ...
            $table_aux['type'] = $output_type;
            $table_aux['title'] = '<span class="badge badge-pill ' . $badge . '">' . $output_type . '</span> <strong>' . $output['output_title'] . '</strong></br><span class=n"abstract_text">' . $output['output_authors'] . '</span>';
            $table_aux['available'] = $available;
            $table_aux['file'] = $file;
            $table_aux['edit'] = $edit;
