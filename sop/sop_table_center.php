    ...
    <div class="optionSelect conceptSheets_optionMenu" id="options_wrapper">
        <div style="float:left">
            ...
            if($person_region['showregion_y'] == '1') {
                echo '<ul class="list-inline" style="padding-left: 10px;padding-top: 7px;">Your region: <li><span style="padding-right:5px">' . $map_region . '</span>' . getTBLCenterUpdatePercentLabel($region_array[$map_region]) . '</li></ul>';
            }
            ?>
        </div>
        ...
        <?php
            foreach ($region_array as $pregion => $percent){
                if($map_region != $pregion){
                    echo '<li style="margin-right: 15px;"><span style="padding-right:5px">'.$pregion.'</span>'.getTBLCenterUpdatePercentLabel($percent).'</li>';
                }
            }
        ?>
    </ul>
