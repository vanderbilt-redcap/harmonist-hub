...
foreach( $RecordSetTableTBLC as $data){
    if(array_key_exists($data['region'],$totalAreasByRegion) && $data['geocode_lat'] != NULL && $data['geocode_lon'] != NULL && $data['geocode_lat'] != '' && $data['geocode_lon'] != '' && ($data['drop_center'] == '' || !in_array($data['drop_center'],$data))) {
        #MAP LOCATIONS
        ...
        $areas["selectedColor"] = adjustColorLightenDarken($colorCountryArray[$data['region']],"30");
        $areas["rollOverColor"] = adjustColorLightenDarken($colorCountryArray[$data['region']],"30");
